...
} = require("./antigravity-cli-adapter.cjs");
const {
...
const { buildMemoryContext } = require("./memory-context.cjs");
const {
  buildChatRetrievalQuery,
  indexChatSources,
  searchChatHistory
} = require("./chat-vector-memory.cjs");
const { buildChatVectorV2Index } = require("./chat-vector-memory-v2.cjs");
const {
  VECTOR_MODEL,
  embedTexts
} = require("./memory-vector.cjs");
const { getLmcStatus, logTelegramTurn } = require("./lmc-memory-store.cjs");
...
function runLmcMemoryIngest(chatId) {
  if (!LMC_MEMORY_ENABLED) {
    ...
    const child = spawn(
      process.execPath,
      [path.join(ROOT, "lmc-memory-ingest.cjs"), "--chat-id", chatId],
      {
        cwd: ROOT,
        env: process.env,
        windowsHide: true,
